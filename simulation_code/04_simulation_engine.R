...
make_pz <- function(n_iv = 30) {
  if (n_iv != 30) {
    stop("make_pz() is only for the main revised Table 1 design with exactly 30 IVs.")
  }
...
make_scenario_grid <- function() {
  iv_strength_levels <- c("very_weak", "weak", "moderate", "strong", "very_strong")
...
run_all_settings <- function(n_sim = 8000,
  ...
  if (nrow(settings) != 132) {
    stop("Expected 132 scenario settings from Table 1, but found ", nrow(settings), ".")
  }
